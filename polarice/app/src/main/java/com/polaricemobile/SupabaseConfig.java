...
public class SupabaseConfig {
    // Configurações do Supabase baseadas no projeto web
    ...
    public static class User {
        public String id;
        public String email;
        public JsonObject user_metadata;
    }
    ...
    // Buscar entregas pendentes do motorista
    public static Delivery[] getPendingDeliveries(String motoristaId, String accessToken) throws IOException {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            ...
            Delivery[] deliveries = new Delivery[jsonArray.size()];
            for (int i = 0; i < jsonArray.size(); i++) {
                deliveries[i] = gson.fromJson(jsonArray.get(i), Delivery.class);
            }
            return deliveries;
        }
    }
    ...
    public static boolean completeDelivery(int deliveryId, String accessToken) throws IOException {
        ...
        updateData.addProperty("status", "concluído");
